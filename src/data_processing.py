...
def store_match_data(
    ...
    """
    Stores data locally, erweitert um StatsBomb-xG.
    """
    if format not in {"spadl", "atomic-spadl"}:
        print("Error: Invalid format. Please choose 'spadl' or 'atomic-spadl'.")
        return None

    with pd.HDFStore(match_data_h5) as store:
        store["competitions"] = competitions        
        store["games"]        = games
        store["teams"]        = teams
        store["players"]      = players[['player_id', 'player_name', 'nickname']].drop_duplicates(
                                   subset='player_id')
        if player_games is not None:
            store["player_games"] = player_games
        else:
            store["player_games"] = players[
                ['player_id', 'game_id', 'team_id', 'is_starter',
                 'starting_position_id', 'starting_position_name',
                 'minutes_played']
            ]

        for game_id, df_actions in actions.items():
            # --- 1) xG-Werte für dieses Spiel aus StatsBomb laden ---
            events = sb.events(
                match_id=game_id,
                split=False,
                fmt="dataframe",
            ...
            shots = events[events['type'] == 'Shot'][['id', 'shot_statsbomb_xg']].copy()
            shots = shots.rename(columns={
                'id': 'original_event_id',
                'shot_statsbomb_xg': 'xG'
            })

            # --- 2) Merge xG in die Aktionen ---
            # Je nach Deinem SPADL-Output könnte die Action-ID anders heißen:
            # hier verwenden wir 'original_event_id' als Schlüssel
            df = df_actions.merge(
                shots,
            ...
            # --- 3) In den HDFStore schreiben ---
            if format == "spadl":
                store[f"actions/game_{game_id}"] = df
            else:  # atomic-spadl
                store[f"atomic_actions/game_{game_id}"] = df

    print(f"Match data ({format}) with xG successfully stored to {match_data_h5}.")
